...
import { SealevelInstructionWrapper, SealevelIgpInstruction, SealevelIgpQuoteGasPaymentInstruction, SealevelIgpQuoteGasPaymentSchema, SealevelIgpQuoteGasPaymentResponseSchema, SealevelIgpQuoteGasPaymentResponse, SealevelHyperlaneTokenDataSchema, SealevelAccountDataWrapper, SealevelOverheadIgpDataSchema, SealevelIgpDataSchema, SealevelInterchainGasPaymasterType, SealevelHypTokenInstruction, SealevelTransferRemoteInstruction, SealevelTransferRemoteSchema, padBytesToLength, isNullish, addressToBytes } from './serialize.js';
...
export class Runner {
    ...
    provider;
    chainName;
    constructor(chain, _provider, addresses) {
        this.chain = chain;
    ...
    async quoteTransferRemoteGas({ destination, sender, }) {
        ...
        const igpPayment = await igp.quoteGasPayment(destination, destinationGas, new PublicKey(sender));
        return { igpQuote: { amount: igpPayment } };
    }
    async populateTransferRemoteTx({ weiAmountOrId, destination, recipient, fromAccountOwner, isNative }) {
        if (!fromAccountOwner)
        ...
        keys.map((k, i) => console.log("KEY: ", k.pubkey.toBase58()));
        ...
        const recentBlockhash = (await this.provider.getLatestBlockhash('finalized')).blockhash;
    ...
    async getTransferInstructionKeyList({ sender, mailbox, randomWallet, igp, isNative }) {
        let keys = [
        ...
        if (igp) {
            keys = [
                ...
                {
                    pubkey: this.deriveGasPaymentPda(igp.programId, randomWallet),
                    isSigner: false,
                    isWritable: true,
                },
            ];
            ...
            if (isNative) {
                keys.push({ pubkey: SystemProgram.programId, isSigner: false, isWritable: false }, // executable
    ...
    deriveGasPaymentPda(igpProgramId, randomWalletPubKey) {
        return this.derivePda(['hyperlane_igp', '-', 'gas_payment', '-', randomWalletPubKey.toBuffer()], igpProgramId);
    }
    deriveIgpProgramPda(igpProgramId) {
        return this.derivePda(['hyperlane_igp', '-', 'program_data'], igpProgramId);
    }
    deriveMailboxOutboxAccount(mailbox) {
        return this.derivePda(['hyperlane', '-', 'outbox'], mailbox);
    }
    deriveMessageDispatchAuthorityAccount() {
        return this.derivePda(['hyperlane_dispatcher', '-', 'dispatch_authority'], this.warpProgramPubKey);
    }
    ...
    deriveAtaPayerAccount() {
        return this.derivePda(['hyperlane_token', '-', 'ata_payer'], this.warpProgramPubKey);
    }
    ...
    getIgpAdapter(tokenData) {
        ...
        if (igpConfig.type === SealevelInterchainGasPaymasterType.Igp) {
            ...
            return new SealevelOverheadIgpAdapter(this.chainName, this.provider, {
    ...
    deriveHypTokenAccount() {
        return this.derivePda(['hyperlane_message_recipient', '-', 'handle', '-', 'account_metas'], this.warpProgramPubKey);
    }
    deriveNativeTokenCollateralAccount() {
        return this.derivePda(['hyperlane_token', '-', 'native_collateral'], this.warpProgramPubKey);
    }
    derivePda(seeds, programId) {
        return this.derivePdaWithBump(seeds, programId)[0];
    }
    derivePdaWithBump(seeds, programId) {
        const [pda, bump] = PublicKey.findProgramAddressSync(seeds.map((s) => Buffer.from(s)), new PublicKey(programId));
        return [pda, bump];
    }
}
class SealevelIgpProgramAdapter {
    chainName;
    provider;
    addresses;
    programId;
    constructor(chainName, provider, addresses) {
...
export class SealevelOverheadIgpAdapter extends SealevelIgpProgramAdapter {
    ...
    addresses;
    overheadIgp;
    constructor(chainName, connection, addresses) {
